...
template <typename T>
class DenseMatrixAlt : public XMLDenseMatrixParser<T> {
public:
    DenseMatrixAlt() : XMLDenseMatrixParser<T>() {}

    DenseMatrixAlt(std::size_t M, std::size_t N) : XMLDenseMatrixParser<T>(M, N) {}
    ...
    DenseMatrixAlt<T> operator+(const DenseMatrixAlt<T>& other) const {
        if (this->M != other.M || this->N != other.N) {
            throw std::invalid_argument("Размеры матриц должны совпадать для сложения.");
        }
        DenseMatrixAlt<T> result(this->M, this->N);
    ...
    DenseMatrixAlt<T> Transpose() const {
        DenseMatrixAlt<T> result(this->N, this->M);
        for (std::size_t i = 0; i < this->M; ++i) {
            for (std::size_t j = 0; j < this->N; ++j) {
                result(j, i) = (*this)(i, j);
            }
    ...
    DenseMatrixAlt<T> ScalarMultiplication(T scalar) const {
        DenseMatrixAlt<T> result(this->M, this->N); 
        for (std::size_t i = 0; i < this->M; ++i) {
            for (std::size_t j = 0; j < this->N; ++j) {
